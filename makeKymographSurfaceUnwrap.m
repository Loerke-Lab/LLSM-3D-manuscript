function [] = makeKymographSurfaceUnwrap(data,CellID,tvec)
...
figure(2)
tiledlayout(1,numel(tvec),'Padding','tight','TileSpacing', 'tight'); 
count = 1;
for t = tvec
    ...
    % crop out specific angle vector
    angleCropStart = 45;
    angleCropEnd = 135;
    angleCropVec = [angleCropStart:5:angleCropEnd];
    angleCropInd = ismember(binsAngle,angleCropVec);
    angleCropResult = result(:,angleCropInd);

    angleCropVertexInd = VertexMatDataA(:,2) <= angleCropEnd & VertexMatDataA(:,2) >= angleCropStart;
    angleCropVertex = VertexMatDataA(angleCropVertexInd,:);


    figure(2)
    %subplot(1,numel(tvec),count)
    nexttile
    imagesc(angleCropVec,binsZ*0.2635,angleCropResult);%colorbar; %caxis(clims)
    %ylabel('Depth (microns)','FontSize',fs)
    %xlabel('Angle (degrees)','FontSize',fs)
    set(gca,'Xlim',[angleCropStart angleCropEnd])
    %xticks([60 75 90 105 120])
    axis off
    hold on
    scatter(angleCropVertex(:,2),angleCropVertex(:,1)*0.2635,9,'k','filled');hold off
    colormap jet
    caxis(clims)
    %set(gca,'Fontsize',20)
    %pause
