function [ExpandMatAll,ContractMatAll] = AreaPropagationFindPeaks_LS_Rho_Wire(data,MovieNums,showEgs)
...
ApicalContHist = NaN(6,Nedges);

for ii=1:6
    DiffBasalCont = spf*(ContractMatAll(ii+10,:,2)-ContractMatAll(ii,:,2));
...
subplot(1,4,1)
%imagesc(edges(1:end-1),(1:16)*1,ApicalContHist);title('Contraction from apical end')
%imagesc(edges,(1:16)*1,ApicalContHist);title('Contraction from apical end')
imagesc(centers,(1:16)*1,ApicalContHist);title('Contraction from apical end')
title('\Delta z = 10 \mum','FontSize',16)
xlabel('\Delta t (s)','FontSize',16)
ylabel('Depth (\mum)','FontSize',16)
xticks([-84 -70 -56 -42 -28 -14 0 14 28 42 56 70 84])
colorbar
caxis([0 0.13])
...
subplot(1,4,3)
%estimate = fitcurveMultiGauss1D_simple(edges(1:end-1),ApicalContHist(3,:),[0.1 -50 22 0.2 50 22]);
estimate = fitcurveMultiGauss1D_simple(centers,ApicalContHist(3,:),[0.1 -20 10 0.2 20 10]);
xlabel('\Delta t (s)','FontSize',16)
ylabel('Probability','FontSize',16)
%title(['\mu_1= ',num2str(estimate(2)),', \mu_2= ',num2str(estimate(5)),', A_1/A_2= ',num2str(estimate(1)*estimate(3)/estimate(4)/estimate(6))])

subplot(1,4,4)
%estimate = fitcurveMultiGauss1D_simple(edges(1:end-1),ApicalContHist(6,:),[0.1 -50 22 0.2 50 22]);
estimate = fitcurveMultiGauss1D_simple(centers,ApicalContHist(6,:),[0.1 -20 10 0.2 20 10]);
xlabel('\Delta t (s)','FontSize',16)
ylabel('Probability','FontSize',16)
%title(['\mu_1= ',num2str(estimate(2)),', \mu_2= ',num2str(estimate(5)),', A_1/A_2= ',num2str(estimate(1)*estimate(3)/estimate(4)/estimate(6))])
