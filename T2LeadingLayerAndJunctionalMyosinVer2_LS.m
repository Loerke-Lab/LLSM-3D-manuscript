function [] = T2LeadingLayerAndJunctionalMyosinVer2_LS(dataSets,intType)
close all
...
for dat=1:Ntype
    ...
    for mn=1:Nmovies
        ...
        for int=1:Nints
            ...
            finite = isfinite(L);
            startL = find(any(finite),1,'first');
            endL   = find(any(finite),1,'last');
...
%             endFrame = min([endCells,endL]);
            ...
            % get intensity data
            I_T1 = squeeze(InterfaceMyo(t1t3ints(int,1),tvec,:))';
            I_T3 = squeeze(InterfaceMyo(t1t3ints(int,2),tvec,:))';
            finiteT1 = isfinite(I_T1);
            finiteT3 = isfinite(I_T3);
            
            IT1i = inpaint_nans(I_T1,2);
            IT1i = filterImage3DpaddedEdges(IT1i, 'Gauss', 3);

            
            IT3i = inpaint_nans(I_T3,2);
            IT3i = filterImage3DpaddedEdges(IT3i, 'Gauss', 3);

            
            I = NaN(Nlayers,numel(tvec));
            
            I(finiteT1) = IT1i(finiteT1);
            I(finiteT3) = IT3i(finiteT3);
            ...
            imagesc(tvecMin,zvec*zstepMic,I);hold on
            M=contour(tvecMin,zvec*zstepMic,LinterpF,[0 0],'LineWidth',2,'Color','k');hold off
            colorbar
            ylabel('Z-depth (microns)','FontSize',fs)
            xlabel('Time (min)','FontSize',fs)
